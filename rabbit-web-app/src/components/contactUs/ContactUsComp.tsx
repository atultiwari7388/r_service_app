...
// Static contact information as fallback
const staticContactInfo = {
  contactMail: "info@rabbitmechanicservices.com",
  contactNumber: "+1 (202) 555-088",
...
export default function ContactUsComp() {
  ...
  const fetchContactUs = async () => {
    ...
    try {
      ...
      if (contactUsSnapshot.exists()) {
        const contactMail =
          contactUsSnapshot.data()?.mail || staticContactInfo.contactMail;
        const contactNumber =
          contactUsSnapshot.data()?.phone || staticContactInfo.contactNumber;
        const address =
          contactUsSnapshot.data()?.address || staticContactInfo.address;
        setContactInfo({ contactMail, contactNumber, address });
      }
    } catch (error) {
      // GlobalToastError(error);
      console.log(error);
      // If there's an error, fall back to static info
      setContactInfo(staticContactInfo);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-100">
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* Contact Information Card - Always shows info (static or from DB) */}
            <div className="bg-white rounded-xl shadow-lg p-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">
                Contact Information
              </h2>
