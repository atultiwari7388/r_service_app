...
export default function MemberJobPage() {
  ...
  //     collection(db, "Users", memberId, "history"),
  //     where("status", "in", [0, -1, 1, 2, 3, 4, 5]),
  ...
  useEffect(() => {
    ...
    // Firestore query from history subcollection
    const jobsQuery = query(
      collection(db, "Users", memberId, "history"),
      where("status", "in", [0, -1, 1, 2, 3, 4, 5]),
      orderBy("orderDate", "desc")
            ...
            <div className="space-y-4">
              {jobs.map((job) => {
                ...
                return (
                  <MemberJobsCard
                    key={job.id}
                    companyNameAndVehicleName={`${job.vehicleNumber || "N/A"}`}
