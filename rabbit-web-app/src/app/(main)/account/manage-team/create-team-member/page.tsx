...
export default function CreateTeamMemberPage() {
  ...
  const confirmCreateMember = async () => {
    ...
    try {
      ...
      await createTeamMember({
        ...
        currentUId: effectiveUserId, // Use the determined effectiveUserId
  ...
  // Fetch vehicles when effectiveUserId changes
  useEffect(() => {
    if (effectiveUserId) {
      fetchVehicles();
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              ...
              {roleDisplayNames.map((displayName, index) => {
                const databaseValue = roleDatabaseValues[index];
                return (
                  <option key={databaseValue} value={databaseValue}>
                    {displayName}
                  </option>
                );
              })}
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full p-2 border rounded"
            />
            {/* Vendor and other staff Specific Fields */}
            ...
            {formData.role !== "Vendor" && formData.role !== "Other Staff" && (
              <>
                <input
                  type="text"
                ...
                {/* Pay Type Selection - Show for non-Vendor and non-SubOwner roles */}
                ...
                {/* Vehicle Assignment - Show for Driver, Manager, Accountant, Other Staff, but NOT for SubOwner */}
                {(formData.role === "Driver" ||
                ...
                {/* Access Controls - Show for non-Vendor roles except SubOwner */}
                {formData.role !== "SubOwner" && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Record Access
            ...
            {/** Don't show password for vendor and other staff */}
            {formData.role !== "Vendor" && formData.role !== "Other Staff" && (
