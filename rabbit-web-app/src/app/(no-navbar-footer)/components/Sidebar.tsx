...
export default function Sidebar({ activeScreen, onNavigate }: SidebarProps) {
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-16 bg-[#F96176] flex flex-col items-center py-6">
      {/* Logo / Home */}
      <div className="mb-10">
        <button
          aria-label="Home"
          className="h-10 w-10 rounded-xl bg-[#F96176] flex items-center justify-center text-white hover:bg-[#F96176] transition"
        >
          <Home size={18} />
        </button>
      </div>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isActive
                  ? "bg-white text-[#F96176]"
                  : "text-white hover:bg-[#F96176] hover:text-white"
              )}
            >
              {item.icon}
