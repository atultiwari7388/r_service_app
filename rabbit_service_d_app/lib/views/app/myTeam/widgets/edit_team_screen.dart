...
//       {super.key, required this.memberId, required this.currentUId});
...
class _EditTeamMemberState extends State<EditTeamMember> {
  ...
  // Helper method to get display name for a database role value
  String _getRoleDisplayName(String databaseValue) {
    int index = roleDatabaseValues.indexOf(databaseValue);
    return index != -1 ? roleDisplayNames[index] : databaseValue;
  }

  // Helper method to get database value for a display name
  String _getRoleDatabaseValue(String displayName) {
    int index = roleDisplayNames.indexOf(displayName);
    return index != -1 ? roleDatabaseValues[index] : displayName;
  }
                    ...
                    Padding(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        items: roleDisplayNames.asMap().entries.map((entry) {
                          ...
                          return DropdownMenuItem<String>(
                            value: databaseValue, // Store the database value
                            child: Text(displayName), // Show display name in UI
                          );
                        }).toList(),
                      ),
                    ),
  ...
  Future<void> _updateMember() async {
    ...
    try {
      ...
      Map<String, dynamic> updateData = {
        ...
        "dateOfHire": dateOfHire,
        "dateOfTermination": dateOfTermination,
        "role": selectedRole, // This will be "SubOwner" for Co-Owner
        "isManager": selectedRole == "Manager",
        "isDriver": selectedRole == "Driver",
        "isVendor": selectedRole == "Vendor",
        "isAccountant": selectedRole == "Accountant",
      ...
      // Update vehicle assignments (only for non-Vendor and non-SubOwner roles)
      if (selectedRole != "Vendor" && selectedRole != "SubOwner") {
        await _updateVehicleAssignments();
      }
